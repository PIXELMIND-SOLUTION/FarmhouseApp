    ...
    try {
      ...
      if (mounted) {
        if (locationProvider.error != null) {
          _showSnackBar(locationProvider.error!, isError: true);
  ...
  Future<void> _handleLocationSelection(Prediction prediction) async {
    try {
      ...
      if (mounted) {
        ...
        // Split address to get shorter version
        List<String> parts = address.split(',');
        String trimmedAddress = parts.length > 1
            ? parts.sublist(0, 2).join(',').trim()
            : address;

        // Update location using provider
        await locationProvider.updateLocation(latitude, longitude);
                  ...
                  children: [
                    ...
                    Consumer<LocationProvider>(
                      builder: (context, locationProvider, child) {
                        if (locationProvider.hasLocation && !_hasSearchStarted) {
